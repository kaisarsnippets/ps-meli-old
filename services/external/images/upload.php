...
$internal_access_token = Configuration::get('MERCADOLIBRE_IMAGES_TOKEN');
...
if ($internal_access_token === $internal_access_token_req) {
    ...
    foreach ($_FILES as $file) {
        ...
        $img_obj->id = $id;
        $img_obj->name = $newfilename;
        $img_obj->ext = $ext;
        $img_obj->dir = $dir;
        $img_obj->path = $path;
        $img_obj->weight = $weight;
        
        array_push($res, $img_obj);
    }
    
    print_r(Tools::jsonEncode($res));
